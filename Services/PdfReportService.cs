...
namespace star_events.Services
{
    ...
    {
        private readonly Font _titleFont = new Font(Font.FontFamily.HELVETICA, 24, Font.BOLD, BaseColor.BLUE);
        private readonly Font _headerFont = new Font(Font.FontFamily.HELVETICA, 18, Font.BOLD);
        private readonly Font _subtitleFont = new Font(Font.FontFamily.HELVETICA, 14, Font.NORMAL, BaseColor.GRAY);
        private readonly Font _normalFont = new Font(Font.FontFamily.HELVETICA, 10, Font.NORMAL);
        private readonly Font _boldFont = new Font(Font.FontFamily.HELVETICA, 10, Font.BOLD);
        private readonly Font _smallFont = new Font(Font.FontFamily.HELVETICA, 8, Font.NORMAL, BaseColor.GRAY);
            ...
            string? eventTitle = null)
        {
            ...
            {
                AddPaymentMethodsSection(document, paymentMethods, totalRevenue);
            }
        ...
        private void AddHeader(Document document, string title, DateTime startDate, DateTime endDate, string? subtitle = null)
        {
            // header
        ...
        private void AddPaymentMethodsSection(Document document, List<PaymentMethodSummary> paymentMethods, decimal totalRevenue)
        {
            ...
            foreach (var method in paymentMethods)
            {
                ...
                table.AddCell(new PdfPCell(new Phrase($"${method.TotalAmount:N2}", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase($"{(method.TotalAmount / totalRevenue * 100):F1}%", _normalFont)) { Border = Rectangle.BOX });
            }
        ...
        private void AddPaymentsTable(Document document, List<Payment> payments)
        {
            ...
            foreach (var payment in payments.Take(100))
            {
                ...
                table.AddCell(new PdfPCell(new Phrase($"${payment.AmountPaid:N2}", _normalFont)) { Border = Rectangle.BOX });
                ...
                table.AddCell(new PdfPCell(new Phrase(payment.PaymentDateTime.ToString("MMM dd, yyyy"), _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(payment.PaymentGatewayTransactionID, _normalFont)) { Border = Rectangle.BOX });
            }
        ...
        private void AddEventsTable(Document document, List<Event> events)
        {
            ...
            foreach (var evt in events.Take(50))
            {
                ...
                table.AddCell(new PdfPCell(new Phrase(evt.StartDateTime.ToString("MMM dd, yyyy"), _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(evt.EndDateTime.ToString("MMM dd, yyyy"), _normalFont)) { Border = Rectangle.BOX });
        ...
        private void AddUsersTable(Document document, List<ApplicationUser> users)
        {
            ...
            {
                table.AddCell(new PdfPCell(new Phrase(user.Id.Substring(0, 8) + "...", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(user.Email ?? "N/A", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase($"{user.FirstName} {user.LastName}", _normalFont)) { Border = Rectangle.BOX });
        ...
        private void AddTicketsTable(Document document, List<Ticket> tickets)
        {
            ...
            foreach (var ticket in tickets.Take(100))
            {
                ...
                table.AddCell(new PdfPCell(new Phrase(ticket.TicketType?.Name ?? "N/A", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase($"${(ticket.TicketType?.Price ?? 0):N2}", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(ticket.TicketType?.Event?.Title ?? "N/A", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(ticket.Booking?.BookingDateTime.ToString("MMM dd, yyyy") ?? "N/A", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(ticket.IsScanned ? "Scanned" : "Unscanned", _normalFont)) { Border = Rectangle.BOX });
                table.AddCell(new PdfPCell(new Phrase(ticket.Booking?.User?.Email ?? "N/A", _normalFont)) { Border = Rectangle.BOX });
            }
        ...
        {
            var footerParagraph = new Paragraph($"Report generated by Star Events System on {DateTime.Now:MMMM dd, yyyy 'at' HH:mm}", _smallFont)
            {
                Alignment = Element.ALIGN_CENTER,
                SpacingBefore = 30
            };
