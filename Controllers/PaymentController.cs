...
namespace star_events.Controllers
{
    ...
    public class PaymentController : Controller
    {
        ...
        // POST: Payment/Export
        [HttpPost]
        public async Task<IActionResult> Export(string exportType, DateTime? startDate, DateTime? endDate, string status = "")
        {
            var currentUserId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            var userRoles = _userRepository.GetRoles(_userRepository.GetById(currentUserId));
        ...
        private string GeneratePaymentsCSV(DateTime? startDate, DateTime? endDate, string status)
        {
            ...
            {
                csv += $"{payment.PaymentID},{payment.AmountPaid},{payment.PaymentMethod},{payment.PaymentStatus},{payment.PaymentDateTime:yyyy-MM-dd HH:mm},{payment.PaymentGatewayTransactionID},{payment.Booking?.BookingID}\n";
            }
            return csv;
        }
        ...
        private string GenerateBookingsCSV(DateTime? startDate, DateTime? endDate, string status)
        {
            ...
            {
                csv += $"{booking.BookingID},{booking.TotalAmount},{booking.Status},{booking.BookingDateTime:yyyy-MM-dd HH:mm},{booking.User?.Email}\n";
            }
            return csv;
        }
    }
