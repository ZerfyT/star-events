...
public class EventController : Controller
{
    ...
    public IActionResult Create(
        [Bind("EventID,LocationID,CategoryID,OrganizerID,Title,Description,StartDateTime,EndDateTime,Status")]
        Event @event, IFormFile[] uploadedImages)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (uploadedImages != null && uploadedImages.Length > 0)
            {
                ...
                for (var i = 0; i < Math.Min(uploadedImages.Length, 10); i++)
        ...
        ViewData["LocationID"] =
            new SelectList(_locationRepository.GetAll(), "LocationID", "Address", @event.LocationID);
    ...
    public IActionResult Edit(int id,
        [Bind(
            "EventID,LocationID,CategoryID,OrganizerID,Title,Description,StartDateTime,EndDateTime,Status,ImagePaths")]
        Event @event, IFormFile[] uploadedImages, string? deletedImageIndexes)
    {
